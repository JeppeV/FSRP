namespace FSRP
    ...
    module Core =
        ...
        let rec private consume_signal (signal: Signal<'B>) (output: 'B -> bool) =
            match signal with
            | hd::tl -> 
                if output hd then
                    do tick()
                    consume_signal (internal_adv tl) output
                else 
                    do store.cleanup()
                    ()
        ...
        let process_signal (sf: Signal<'A> -> Signal<'B>) (input: unit -> 'A) (output: 'B -> bool) =
            let input_signal = make_input_signal input
            let output_signal = sf input_signal
            consume_signal output_signal output

        let unfold_signal (signal: Signal<'A>) (output: 'A -> bool) =
    ...
    module Library =
        ...
        [<FSRP>]
        let rec constant (a: Box<'a>) =
            unbox a :: delay(lazy(constant a))
        ...
        [<FSRP>]
        let rec scan (f: Box<Box<'b> -> 'a -> Box<'b>>) (acc: Box<'b>) (i: Signal<'a>) : Signal<'b> =
            match i with 
            | a :: tl ->
        ...
        //[<FSRP>]
        //let rec leakyNats2 () =
        //    let unstable_in_now = (fun f -> 1 + 1)
        //    0 :: delay(lazy(map (box(lazy(fun i -> i + 1))) (leakyNats2 ())))
        ...
        //let rec map (f: Box<'A -> 'B>) ((hd :: tl): Signal<'A>) : Signal<'B> =
        //    ((unbox f) hd) :: delay(map (adv tl))
